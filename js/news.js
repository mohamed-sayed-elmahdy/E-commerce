...
const newsContent = {
  english: {
    title: "Our <span>N</span>ews",
    items: [
      {
        title: "Home Maintenance Tasks You Need to Tackle in November",
        date: "15 Nov, 2018",
        text: "We’ll be honest: We hate the phrase “transitional month.” Instead, we like to think of November as a large-scale dress rehearsal for winter—the month that’s cold but not yet bone-chilling, frosty but not yet snowy.",
        linkText: "Read More",
        linkHref: "#",
      },
      {
  ...
  arabic: {
    ...
    items: [
      {
        title: "مهام صيانة المنازل التي تحتاج إلى إنجازها في نوفمبر",
        date: "15 نوفمبر 2018",
        text: "بصراحة، نحن نكره عبارة 'شهر انتقالي'. بدلاً من ذلك، نعتقد أن نوفمبر هو بروفة كبيرة للشتاء - الشهر الذي يكون باردًا ولكنه ليس قارسًا، صقيعًا ولكنه ليس ثلجيًا بعد.",
...
function UpdateContent(lang) {
  ...
  //  Update News
  const container = document.querySelector(".content-inner .container");

  // Update the title
  container.querySelector("h1").innerHTML = newsContent[lang].title;

  // Update each news row
  const newsRows = document.querySelectorAll(".news-item .news-row");

  newsRows.forEach((newsRow, index) => {
    if (newsContent[lang].items[index]) {
      // Update the title
      const newsTitle = newsRow.querySelector(".news-title");
      newsTitle.textContent = newsContent[lang].items[index].title;

      // Update the date
      const newsDate = newsRow.querySelector(".news-date");
      newsDate.innerHTML = `<i class="far fa-calendar-alt"></i> ${newsContent[lang].items[index].date}`;

      // Update the text
      const newsText = newsRow.querySelector(".news-text");
      newsText.textContent = newsContent[lang].items[index].text;

      // Update the "Read More" link
      const readMoreLink = newsRow.querySelector(".btn-read-more");
      readMoreLink.textContent = newsContent[lang].items[index].linkText;
      readMoreLink.href = newsContent[lang].items[index].linkHref;
    }
  });
}
