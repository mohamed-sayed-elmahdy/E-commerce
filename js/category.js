...
function UpdateContent(lang) {
  ...
  document.querySelector(".custom-container h1").innerHTML =
    categoryContent[lang].title;

  // Update Cards
  const cardElements = document.querySelectorAll(".card");
  categoryContent[lang].cards.forEach((card, index) => {
    const cardElement = cardElements[index];
    const imgElement = cardElement.querySelector(".card-image");
    const titleElement = cardElement.querySelector(".card-border h2");

    // Update Image and Title
    imgElement.alt = card.imgAlt;
