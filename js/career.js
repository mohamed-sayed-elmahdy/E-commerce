...
const careerContent = {
  english: {
    ...
    fileLabel: "Upload CV (PDF file, 3MB max):",
    submitButton: "Submit",
    applyButton: "Apply",
  ...
  arabic: {
    ...
    fileLabel: "ارفع السيرة الذاتية (ملف PDF، بحد أقصى 3MB):",
    submitButton: "إرسال",
    applyButton: "قدم الآن",
    toggleDetails: "تفاصيل",
    jobCards: [
      {
        title: "مدير مشروع أول.",
        region: "المنطقة: الأردن - عمان",
        ref: "SPM-004-JD",
        details: "لوريم ايبسوم هو نص شكلي يُستخدم في صناعات الطباعة والنشر.",
      },
      {
        title: "منسق مشروع",
        region: "المنطقة: الأردن - عمان",
        ref: "PC-005-JD",
        details:
          "تم تطوير هذا النص ليكون مثالاً قابلاً للتطبيق في مجالات الطباعة والنشر.",
      },
      {
        title: "مطور مبتدئ",
        region: "المنطقة: الأردن - عمان",
        ref: "JD-006-JD",
        details: "نص تجريبي يوضح كيفية استخدام النصوص المشابهة في المواقع.",
      },
    ],
  },
};
...
function UpdateContent(lang) {
  ...
  // Career Content
  // Update Title
  document.querySelector(".job-listings .section-title").textContent =
    careerContent[lang].sectionTitle;

  // Update Job Cards
  const jobCardContainers = document.querySelectorAll(".job-card");
  careerContent[lang].jobCards.forEach((job, index) => {
    const jobCard = jobCardContainers[index];
    jobCard.querySelector(
      ".job-header-container p"
    ).innerHTML = `${job.title}<br>${job.region}`;
    jobCard.querySelector(".toggle-details-btn").textContent =
      careerContent[lang].toggleDetails;
    jobCard.querySelector(".job-details").textContent = job.details;
  });

  // Update Apply Buttons
  const applyButtons = document.querySelectorAll(".apply-btn");
  applyButtons.forEach((btn) => {
    btn.textContent = careerContent[lang].applyButton;
  });

  // Update Form Title
  document.querySelector(".submit-form .form-title").textContent =
    careerContent[lang].formTitle;

  // Update Form Placeholders
  const formFields = document.querySelectorAll(".submit-form .input-field");
  formFields[0].placeholder = careerContent[lang].placeholders.name;
  formFields[1].placeholder = careerContent[lang].placeholders.mobile;
  formFields[2].placeholder = careerContent[lang].placeholders.email;

  // Update Message Placeholder
  document.querySelector(".submit-form .textarea-field").placeholder =
    careerContent[lang].placeholders.message;

  // Update File Label
  document.querySelector(".submit-form .file-label").textContent =
    careerContent[lang].fileLabel;

  // Update Submit Button
  document.querySelector(".submit-form .submit-btn").textContent =
    careerContent[lang].submitButton;
}
